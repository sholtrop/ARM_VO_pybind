...
#pragma omp parallel sections
  {
#pragma omp section
    F = cv::findFundamentalMat(prev_keypoints, curr_keypoints, mask, 8, 1,
                               0.99); // FM_RANSAC
#pragma omp section
  ...
  GRIC(prev_keypoints, curr_keypoints, prev_keypoints.size(), F, H, 0.8,
       Fcriteria, Hcriteria);
  ...
  if (Fcriteria < Hcriteria) {
    ...
    F = cv::findFundamentalMat(prev_inliers, curr_inliers, mask, 8, 0.5,
                               0.99); // FM_RANSAC

    cv::Mat R, t, _3dPoints;
    recoverPose(F, prev_inliers, curr_inliers, cameraMatrix, R, t, mask,
                _3dPoints);
    ...
    // std::cout << "Position: [" << t_f.at<float>(0) << ", " <<
    // t_f.at<float>(1)
    //           << ", " << t_f.at<float>(2) << "]" << std::endl;
  } else {
  }
  // std::cerr << "Skipping " << Fcriteria << " " << Hcriteria << std::endl;
}
