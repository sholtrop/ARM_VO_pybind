...
// const uint8_t __attribute__ (( aligned (16) )) _Powers [16] = { 1, 2, 4, 8,
// 16, 32, 64, 128, 1, 2, 4, 8, 16, 32, 64, 128 };
#pragma pack(16)
const uint8_t _Powers[16] = {1, 2, 4, 8, 16, 32, 64, 128,
                             1, 2, 4, 8, 16, 32, 64, 128};
#pragma pack()
static uint8x16_t powers = vld1q_u8(
    _Powers); //~ Set the powers of 2 (do it once for all , if applicable )
...
uint16_t movemask(uint8x16_t input) {
  ...
  uint64x2_t _Mask =
      vpaddlq_u32(vpaddlq_u16(vpaddlq_u8(vandq_u8(input, powers))));
...
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
void FAST9_NEON(const cv::Mat &img, std::vector<cv::KeyPoint> &keypoints,
                int threshold) {
  const int K = 8, N = 25;
  ...
  uint8x16_t delta = vdupq_n_u8(-128), t = vdupq_n_u8((char)threshold),
             K16 = vdupq_n_u8((char)K);
  (void)K16;
  (void)delta;
  (void)t;
  ...
  for (i = -255; i <= 255; i++)
    threshold_tab[i + 255] = (uchar)(i < -threshold  ? 1
                                     : i > threshold ? 2
                                                     : 0);

  cv::AutoBuffer<uchar> _buf(
      (img.cols + 16) * 3 * (sizeof(int) + sizeof(uchar)) + 128);
  uchar *buf[3];
  buf[0] = _buf;
  ...
  for (i = 3; i < img.rows - 2; i++) {
    ...
    for (k = 0; k < ncorners; k++) {
      ...
      if ((score > prev[j + 1] && score > prev[j - 1] && score > pprev[j - 1] &&
        ...
        keypoints.push_back(
            cv::KeyPoint((float)j, (float)(i - 1), 7.f, -1, (float)score));
      }
    }
  }
}
