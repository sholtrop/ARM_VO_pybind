...
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
// void Viewer::show(const cv::Mat &frame, const std::vector<cv::Point2f>&
// prev_keypoints,
//                  const std::vector<cv::Point2f>& curr_keypoints, const int
//                  FPS, const cv::Mat& translation)
//{
//    cv::Mat frameWithFeatures;
//    if (curr_keypoints.size() > 0)
//        frameWithFeatures = draw_tracked_kpts(frame, prev_keypoints,
//        curr_keypoints);
//    else
...
//    putText(frameWithFeatures, text, cv::Point(8,30), 0, 1.0,
//    cv::Scalar(0,0,255), 3);
//
...
//    putText(trajectory, text, cv::Point(350,20), 0, 0.6, cv::Scalar::all(255),
//    2);
//
//    sprintf(text, "Y[m]: %.2f", path.back().y);
//    putText(trajectory, text, cv::Point(350,50), 0, 0.6, cv::Scalar::all(255),
//    2);
//
...
cv::Mat
Viewer::show(cv::Mat &frame,
             py::array_t<float, py::array::c_style | py::array::forcecast>
                 prev_keypoints,
             py::array_t<float, py::array::c_style | py::array::forcecast>
                 curr_keypoints,
             int FPS, cv::Mat &translation) {
  if (prev_keypoints.ndim() != 2)
    throw std::runtime_error("Input should be 2-D NumPy array");

  std::vector<float> pos_prev(prev_keypoints.size()),
      pos_curr(curr_keypoints.size());
  std::vector<cv::Point2f> pos_prev_pts, pos_curr_pts;
  std::memcpy(pos_prev.data(), prev_keypoints.data(),
              prev_keypoints.size() * sizeof(float));
  std::memcpy(pos_curr.data(), curr_keypoints.data(),
              curr_keypoints.size() * sizeof(float));
  ...
  //     frameWithFeatures = draw_tracked_kpts(frame, pos_prev_pts,
  //     pos_curr_pts);
  //   else
  //     cv::cvtColor(frame, frameWithFeatures, 8);
...
// -------------------------------------------------------------------------------------
cv::Mat
Viewer::draw_tracked_kpts(const cv::Mat &frame,
                          const std::vector<cv::Point2f> &prev_keypoints,
                          const std::vector<cv::Point2f> &curr_keypoints) {
  cv::Mat frameWithFeatures;
  cv::cvtColor(frame, frameWithFeatures, 8);

  for (size_t i = 0; i < curr_keypoints.size(); i++) {
    cv::line(frameWithFeatures, prev_keypoints[i], curr_keypoints[i],
             cv::Scalar(255, 0, 0), 2);
