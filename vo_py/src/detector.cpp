...
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
void gridFASTdetector::setMaxFeatures(const int n) { nFeatures = n; }

// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
void gridFASTdetector::setThreshold(const int thresh) { threshold = thresh; }
...
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
bool gridFASTdetector::detect(const cv::Mat &image,
  ...
  cv::parallel_for_(cv::Range(0, nRows * nCols),
                    GridAdaptedFeatureDetectorInvoker(image, keypoints,
                                                      maxPerCell, nRows, nCols,
                                                      threshold, &kptLock));

  if (keypoints.size() < 100) {
    // std::cout << "Few keypoints in the scene! Lowering FAST threshold to 5
    // for this frame. ";
...
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
GridAdaptedFeatureDetectorInvoker::GridAdaptedFeatureDetectorInvoker(
    const cv::Mat &image, std::vector<cv::KeyPoint> &keypoints,
    const int maxPerCell, const int gridRows, const int gridCols,
    const int threshold, cv::Mutex *kptLock)
    : gridRows_(gridRows), gridCols_(gridCols), maxPerCell_(maxPerCell),
      threshold_(threshold), keypoints_(keypoints), image_(image),
      kptLock_(kptLock) {}
...
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
void GridAdaptedFeatureDetectorInvoker::operator()(
    const cv::Range &range) const {
  for (int i = range.start; i < range.end; ++i) {
    int celly = i / gridCols_;
    int cellx = i - celly * gridCols_;

    cv::Range row_range((celly * image_.rows) / gridRows_,
                        ((celly + 1) * image_.rows) / gridRows_);
    cv::Range col_range((cellx * image_.cols) / gridCols_,
                        ((cellx + 1) * image_.cols) / gridCols_);
    ...
    keypoints_.insert(keypoints_.end(), sub_keypoints.begin(),
                      sub_keypoints.end());
  }
}
